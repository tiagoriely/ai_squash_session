# evaluation/generation/evaluate_rrf_pipeline.py
...
OUTPUT_DIR = "evaluation/evaluation_reports/RAGAS/rrf_hybrid"
TOP_K_RETRIEVAL = 5
RRF_K_CONSTANT = 60
...
def run_full_rag_pipeline(
    ...
    for i, query in enumerate(queries, 1):
        ...
        # MODIFIED: Call the RRF hybrid retriever
        print("  - 🔎 Retrieving documents (RRF Hybrid)...")
        retrieved_results = hybrid_search_rrf(
            user_query=query,
        ...
        if not contexts:
            ...
            contexts, answer = [], "No relevant documents were found to generate an answer."
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run a full RRF Hybrid RAG pipeline evaluation using RAGAS.")
    parser.add_argument("--limit", type=int, default=5,
                        help="Number of prompts to evaluate. Set to -1 for all. (Default: 5)")
    args = parser.parse_args()

    print("🚀 Starting RRF Hybrid RAG Pipeline Evaluation...")
    print("\n--- 📂 Loading assets ---")
    ...
    # MODIFIED: Update the pipeline name for output files
    pipeline_name = "RRF_Hybrid"
