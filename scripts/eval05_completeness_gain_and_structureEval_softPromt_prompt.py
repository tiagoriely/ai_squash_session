...
if __name__ == "__main__":
    ...
    for query_info in queries_to_evaluate:
        ...
        # --- STAGE 3: EVALUATION with CompletenessGain + PSA Flexible ---
        ...
        psa_resp = calculate_psa_flexible(target_plan, PROJECT_ROOT / EBNF_GRAMMAR_PATH)
        psa_base = calculate_psa_flexible(baseline_plan, PROJECT_ROOT / EBNF_GRAMMAR_PATH)

        # --- NEW: Run the custom complex check ---
        ...
        combined_scores = {
            ...
            'psa_resp': psa_resp,
            'psa_base': psa_base,
            'custom_resp': float(custom_resp),  # Convert boolean to float for summary stats
            'custom_base': float(custom_base)
        }

        print(f"  -> Combined Scores: {combined_scores}")
        query_results.append(combined_scores)

    # --- Final Summary ---
    print("\n" + "=" * 80)
    print("Final Combined Evaluation Summary (Light Prompt)")
    print("=" * 80)
