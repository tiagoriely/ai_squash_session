...
if __name__ == "__main__":
    ...
    json_files_to_process = list(TARGET_DIR.glob('*.json'))

    if not json_files_to_process:
        print(f"🤷 No JSON files found in '{TARGET_DIR}'. Nothing to process.")
        sys.exit(0)

    print(f"Found {len(json_files_to_process)} JSON file(s) to process in the target directory.")

    # MODIFIED: Loop through each found file and run the evaluation
    for filepath in json_files_to_process:
        # Print a clear separator for each file's output
        print("\n\n" + "#" * 80)
        print(f"   PROCESSING FILE: {filepath.name}")
        print("#" * 80)
        run_combined_diversity_evaluation(filepath)
