# scripts/rag02_test01_simple_prompt_size100.py
import yaml
import json
...
if __name__ == "__main__":
    ...
    CORPUS_SIZE = 100
    TOP_K_CONTEXT = 3  # This is 'k', which we will vary in later experiments
    ...
    queries_for_evaluation = [
        {"query_id": "qual_specific",
         "text": "a 45-minute conditioned game session for an advanced player focusing on volley drops"},
        {"query_id": "qual_vague", "text": "a session to improve my forehand"},
        {"query_id": "qual_ooc", "text": "a solo session to practice cross drops"},
        {"query_id": "qual_ood_duration", "text": "a 90-minute drill session for an advanced player"},
        {"query_id": "qual_shotside", "text": "a 45-minute drill focusing only on the backhand"}
    ]
    ...
    # Outer loop iterates through each grammar type
    ...
    output_filename = f"human_evaluation_set_k{TOP_K_CONTEXT}_size{CORPUS_SIZE}.json"
