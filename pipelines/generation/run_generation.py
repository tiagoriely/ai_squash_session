...
def infer_type(query: str) -> str:
    ...
    return answer if answer in PATTERNS else "conditioned_game"
...
if __name__ == "__main__":
    ...
    for i, doc in enumerate(docs, 1):
        print(f"{i:2d}. id={doc['id']:<4} {shorten(doc['contents'])}")

    print("\n=== Answer ===\n")
    context = build_context(docs)
    session_type = infer_type(args.query)

    prompt_path = Path("prompts") / f"session_{session_type}.txt"
