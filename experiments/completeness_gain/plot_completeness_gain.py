...
Conventions:
- X-axis (lines): corpus sizes [100, 200, 300, 400, 500]
- Y-axis: metric score
...
def main() -> None:
    ...
    ax.set_title("Average Completeness Gain vs Corpus Size")
    ax.set_xlabel("Corpus size")
    ax.set_ylabel("Average Completeness Gain")
    ...
    ax.set_title("Average c_resp vs Corpus Size")
    ax.set_xlabel("Corpus size")
    ax.set_ylabel("Average c_resp")
    ax.set_xticks(x_sizes)
    ax.grid(True, linestyle="--", alpha=0.3)

    # Single legend for figure 1
    ...
    ax2.set_xticklabels(draw_order)
    ax2.set_ylabel("Average score")
    ax2.set_title("Average c_resp vs c_base by Corpus Type (Averaged over Sizes)")
    ...
    ax2.legend(handles=[proxy_resp, proxy_base], frameon=True, loc="upper left")
