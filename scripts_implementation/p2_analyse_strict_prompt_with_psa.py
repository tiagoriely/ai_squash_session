# scripts_implementation/p2_analyse_strict_prompt_with_psa.py
...
# Updated import to use the score fusion strategy
...
# Only import the required PSA metric
...
if __name__ == "__main__":
    ...
    queries_to_evaluate = [
        {"query_id": "complex_45cg_adv_volley",
         "text": "a 45-minute conditioned game session for an advanced player focusing on volley drops"},
        {"query_id": "complex_60mix_straight_lob",
         "text": "a 60-minute mix session for an professional player focusing on straight lob"},
        {"query_id": "vague_forehand", "text": "generate a session to improve my forehand"},
        {"query_id": "squash_other_squash_rules", "text": "rules of a tie-break in squash"},
        {"query_id": "ooc_solo_cross_drop", "text": "a solo to practice cross drops"},
    ...
    for query_info in queries_to_evaluate:
        ...
        # This is the simple/soft prompt you wanted to test first
        prompt_template = """You are an expert squash coach AI that generates session plans. You MUST follow all formatting instructions precisely.

        Your task is to generate a detailed squash session plan based ONLY on the information provided in the "CONTEXT" section.

        ---
        **STRUCTURE REQUIREMENTS**

        The final output must be a single, complete session plan. You MUST structure the plan using the following markdown headers, exactly as shown in this example:

        ### Warm-up
        - [Details of warm-up exercises here]

        ### Activity
        - [Details of the first activity block here]

        ### Activity
        - [Details of the second activity block here]
        ---
        ...
        # The evaluation stage is now simplified to only calculate PSA
        print("  -> Evaluating with Flexible PSA...")

        psa_score = calculate_psa_flexible(generated_plan, PROJECT_ROOT / EBNF_GRAMMAR_PATH)

        # Store the simplified results
        result = {
            'query_id': query_id,
            'psa_score': psa_score
        }
