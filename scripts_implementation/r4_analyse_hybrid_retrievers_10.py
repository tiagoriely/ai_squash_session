...
from rag.retrieval_fusion.strategies import (
    dynamic_query_aware_rrf,
...
def analyse_hybrid_strategy(
    ...
    if strategy_name == 'static_weighted_rrf':
        # Baseline strategy prioritising the Field retriever
        static_weights = {'field_metadata': 0.6, 'sparse_bm25': 0.3, 'semantic_e5': 0.1}
        fused_results = _rrf(standalone_results, static_weights)

    elif strategy_name == 'standard_unweighted_rrf':
        # Unweighted, rank-only fusion
        equal_weights = {'field_metadata': 1.0, 'sparse_bm25': 1.0, 'semantic_e5': 1.0}
        fused_results = _rrf(standalone_results, equal_weights)

    elif strategy_name == 'dynamic_query_aware_rrf':
        # Original dynamic query-aware RRF strategy (rank-based)
        fused_results = dynamic_query_aware_rrf(standalone_results, query_text, field_scoring_config)

    elif strategy_name == 'dynamic_query_aware_score_fusion':
        # Score-based dynamic fusion strategy (uses raw scores and dynamic weights)
        fused_results = dynamic_query_aware_score_fusion(standalone_results, query_text, field_scoring_config)
